export default class Helper {
  ...
  getResizedRenderResolution () {
    ...
    if (window.innerWidth >= 7600) {
      renderResolution.divider = 1.6
    } else if (window.innerWidth >= 3800) {
      renderResolution.divider = 1.4
    } else if (window.innerWidth >= 2500) {
      renderResolution.divider = 1.2
    } else if (window.innerWidth >= 1900) {
      renderResolution.divider = 1.1
    }

    renderResolution.renderWidth = Math.floor(window.innerWidth / renderResolution.divider)
    renderResolution.renderHeight = Math.floor(window.innerHeight / renderResolution.divider)
  ...
  onModeChosen(event, isMetaverse = false) {
    event.preventDefault()

    window.isMetaverse = isMetaverse
    document.getElementById('mode').style.display = 'none'
    document.getElementById('launcher').style.display = 'block'
    document.getElementById('warning-pc').style.visibility = 'visible'
  }
